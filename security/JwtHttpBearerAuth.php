 ...
 *     return [
 *         'bearerAuth' => [
 *             'class' => \enterprise\security\JwtHttpBearerAuth::class,
 *         ],
 *     ];
 * }
 * ```
...
class JwtHttpBearerAuth extends AuthMethod
{
    ...
    {
        parent::init();
        if (false === ($this->jwt instanceof Jwt)  && \Yii::$app->has($this->jwt)) {
            $this->jwt = \Yii::$app->get($this->jwt);
        } else {
            $message = 'Missing required components: {params}';
            throw new InvalidParamException(\Yii::t('yii', $message, ['params' => Jwt::class]));
        }
    }
    ...
    public function challenge($response)
    {
        ...
        $response->getHeaders()->set('WWW-Authenticate', $message);
    }
